...
function redisClient() {
  ...
  // Vercel's "Upstash for Redis" Marketplace integration sometimes injects
  // KV_REST_API_URL/KV_REST_API_TOKEN (legacy naming kept for compatibility
  // with the old Vercel KV product) instead of Upstash's own
  // UPSTASH_REDIS_REST_URL/UPSTASH_REDIS_REST_TOKEN — accept either so this
  // doesn't depend on which one the integration happened to set.
  const url = process.env.UPSTASH_REDIS_REST_URL || process.env.KV_REST_API_URL;
  const token = process.env.UPSTASH_REDIS_REST_TOKEN || process.env.KV_REST_API_TOKEN;
  if (!url || !token) {
    throw new Error(
      "Missing Redis credentials: set UPSTASH_REDIS_REST_URL/UPSTASH_REDIS_REST_TOKEN " +
      "(or KV_REST_API_URL/KV_REST_API_TOKEN) in your Vercel project's environment variables."
    );
  }
